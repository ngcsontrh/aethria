...
public sealed class MentorChatCommandHandler : IStreamRequestHandler<MentorChatCommand, Result<ChatStreamResponse>>
{
    ...
    private readonly IChatAgent _mentorChatAgent;
    private readonly IUnitOfWork _unitOfWork;

    public MentorChatCommandHandler(
        IMentorRepository mentorRepository,
        IChatSessionRepository chatSessionRepository,
        [FromKeyedServices("mentor-chat")] IChatAgent mentorChatAgent,
        IUnitOfWork unitOfWork)
    {
        _mentorRepository = mentorRepository;
        _chatSessionRepository = chatSessionRepository;
        _mentorChatAgent = mentorChatAgent;
        _unitOfWork = unitOfWork;
    }

    public async IAsyncEnumerable<Result<ChatStreamResponse>> Handle(
        MentorChatCommand command,
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        if (mentor is null)
        {
            yield return Result.Fail<ChatStreamResponse>(new NotFoundError("Mentor", command.MentorId.ToString()));
            yield break;
        }

        if (mentor.UserId != command.UserId)
        {
            yield return Result.Fail<ChatStreamResponse>(new NotFoundError("Mentor", command.MentorId.ToString()));
            yield break;
        }
        ...
        if (command.SessionId.HasValue)
        {
            ...
            if (existingSession.UserId != command.UserId ||
                existingSession.MentorId != command.MentorId ||
                existingSession.ResourceId != null)
            {
                yield return Result.Fail<ChatStreamResponse>(new NotFoundError("ChatSession", command.SessionId.Value.ToString()));
                yield break;
            }
        ...
        var tools = mentor.Tools?.Select(t => t.Value).ToList() ?? [];
        ...
        using var timeoutScope = ChatAgentTimeoutScope.Start(
            ...
            _maxStreamDuration,
            cancellationToken);
        await using var enumerator = _mentorChatAgent
        ...
        {
            throw new InvalidOperationException("Mentor chat stream completed without response messages.");
        }
